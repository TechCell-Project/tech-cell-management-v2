...
export const ModelCreateUpdateForm = memo(({
  model,
  handleCloseModel,
}: Readonly<SpuModelCreateUpdateForm>) => {
    ...
    <Form {...updateSpuModelForm}>
      <form
        onSubmit={handleSubmit((data) => {
          ...
          return mutateAsync({
            models: [{ ...data, attributes: mapCommonAttr, slug: convertSlugify(data.name, '-') }],
          });
        })}
      >
        ...
        <div className="w-full flex justify-end gap-4 mt-7">
          <Button variant="ghost" type="button" onClick={handleCloseModel}>
            Quay lại
          </Button>
          <Button
